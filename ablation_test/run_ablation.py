"""
Orchestrates the ablation study's 5 new training runs — see ablation_description.md for the
hypotheses and full experiment design: capacity-matched individual controls for Amazon and
Rangeland, and three pairwise multi-domain pretrain-only runs.

The matched-seed anchor (full 3-domain pretrain, seed=1) is NOT a new run here — it's exactly
`02_train.py --stage pretrain --flux-only --seed 1` with no --domains override, which is
byte-identical to a command already run as part of the completed 5-seed publication sweep
(run_seed_sweep.py). Its checkpoint/metrics already exist at
outputs/multi_domain/{models,evaluation}/pretrained_fluxonly_seed1/ — reuse those directly
rather than rerunning (rerunning would silently overwrite that published production artifact).
...
on vm-sandeep), fail-fast on the first error. Single seed (seed=1) throughout, flux-only target
set — see ablation_description.md for why.
...
Usage:
  python ablation_test/run_ablation.py                                # all 5 runs
  python ablation_test/run_ablation.py --runs capacity_amazon capacity_rangeland
"""
...
def run_capacity_amazon(log_path: Path) -> None:
...
def run_capacity_rangeland(log_path: Path) -> None:
...
def _run_multi_domain_pretrain(domains: str, log_path: Path) -> None:
    ...
    deliberately NOT run here (it already exists as production seed 1)."""
...
def run_pairwise_arctic_amazon(log_path: Path) -> None:
    _run_multi_domain_pretrain("arctic,amazon", log_path)


def run_pairwise_arctic_rangeland(log_path: Path) -> None:
    _run_multi_domain_pretrain("arctic,rangeland", log_path)


def run_pairwise_amazon_rangeland(log_path: Path) -> None:
    _run_multi_domain_pretrain("amazon,rangeland", log_path)
...
def main() -> None:
    ...
    parser.add_argument("--runs", choices=list(RUNS), nargs="+", default=list(RUNS),
                        help="Which of the 5 ablation runs to execute (default: all).")
