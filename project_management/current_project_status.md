...
| multi_domain | Preprocessing | Yes | Spec (`multi_description.md`) reconciled with the merged Arctic/Amazon/Rangeland pipelines; `--flux-only` support + `pretrained`/`finetuned` output-folder split added to `01-04` scripts. Dev-mode smoke test on `vm-cpu-sandeep` not yet run. |
...
`multi_description.md` rewritten: Arctic's real artifact naming (`train_500K_s400.pkl`, not a plain `train.pkl`), a new "Flux-Only Variant" section, removed the stale production-TBD hedge (production config already matches Arctic's architecture, as intended), fixed the `batch_size` pseudocode/code mismatch, marked `compare_models.py` explicitly deferred/unbuilt. Added `--flux-only` to `02_train.py`/`03_predict.py`/`04_evaluate.py`/`run_multi_domain.py` (new shared `domains/multi_domain/flux_only.py` module — Arctic reorders via `arctic_domain/_naming.py`'s existing helpers, Rangeland truncates to its first 4 trailing target columns, Amazon unaffected). Restructured pretrain/finetune outputs into separate `pretrained[_fluxonly]/{domain}/` and `finetuned[_fluxonly]/{domain}/` subfolders (was flat `stage1_*`/`stage2_*`-prefixed filenames) across `models/`, `predictions/`, `evaluation/`, per the user's request for easy stage × variant comparison later.

**8-angle code review (high effort) ran before touching the VM — found and fixed a real, severe bug**: the training loop was windowing Arctic's `train_500K_s400.pkl` at the global `cfg.preprocessing.stride` (1) instead of the pkl's own sidecar stride (400), which would have inflated one epoch to ~200M windows and made training computationally intractable on `vm-sandeep`. Fixed by reading Arctic's stride per-pkl from its sidecar (`domains/arctic_domain/_naming.py`'s `load_stride_seq_len`), matching how the individual Arctic pipeline itself already does this. Also fixed: a doc/code drift in the Outputs table, a predictions-folder collision risk for the pretrain stage, `ARCTIC_FULL_TARGET_ORDER` hardcoded instead of derived from config (now matches Arctic's own `02_train.py` convention), no fail-loud default for an unrecognized domain in `apply_flux_only`, and consolidated 5+ places where stage-folder-path/target-name-selection logic had been re-implemented per-script into the shared `flux_only.py` module. Per the user's separate request, Arctic's multi-domain evaluation now also exports a 50-pixel deterministic prediction sample using the *exact same seed and site selection* as the individual Arctic pipeline (`sample_test_pixels`/`save_prediction_sample` moved from `arctic_domain/04_evaluate.py` into the shared `arctic_domain/_naming.py` so both pipelines call the same implementation) — avoids saving dense per-pixel predictions for the full test set while keeping the two runs directly comparable at identical sites.

### NEXT

1. Dev-mode smoke test on `vm-cpu-sandeep`: `01_preprocess.py`, `02_train.py --stage {pretrain,finetune}` for both the full-target and `--flux-only` variants, `03_predict.py`, `04_evaluate.py`. Confirm all three domains' real preprocessed files (Arctic `train_500K_s400.pkl` + grid-split `val.pkl`/`test.pkl`, Amazon `train.pkl`/`val.pkl`/`test.pkl`) are actually present on the VM before running.
2. Production run on `vm-sandeep` — only after the dev test is confirmed clean and reviewed; separate go-ahead required.
